...
const Auth = () => {
  ...
  const { user, signIn, signUp, resetPassword, updatePassword, signInWithGoogle, loading: authLoading } = useAuth();
  const [searchParams] = useSearchParams();
  const [mode, setMode] = useState<'signin' | 'signup' | 'forgot-password' | 'reset-password'>(
    searchParams.get('mode') === 'signup' ? 'signup' : 
    searchParams.get('mode') === 'forgot-password' ? 'forgot-password' :
    searchParams.get('mode') === 'reset-password' ? 'reset-password' : 'signin'
  );
  ...
  // Redirect if already logged in (except for reset-password mode)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    else if (urlMode === 'forgot-password') setMode('forgot-password');
    else if (urlMode === 'reset-password') setMode('reset-password');
    else setMode('signin');
  }, [searchParams]);
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (error) {
        console.error('[Auth Page] Google signin error:', error);
        toast.error(error.message);
      }
    } catch (error) {
      console.error('[Auth Page] Unexpected Google signin error:', error);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('[Auth Page] Submit error:', error);
  ...
  const getSubtitle = () => {
    switch (mode) {
      ...
      case 'signup': return 'Join BLAQROTH for a personalized experience';
          ...
          >
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                {/* Password field - show for signin, signup, and reset-password */}
